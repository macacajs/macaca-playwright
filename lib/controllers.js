...
const extraActions = require('./extra-actions');
...
controllers.click = async function(elementId) {
  ...
  if (isNaN(Number(elementId))) {
    return await this.page.click(...arguments);
  }
 ...
 * @return {Promise}
 */
controllers.extra = async function(funcName, params = []) {
  if (funcName) {
    return await extraActions[funcName].call(this, params);
  }
};
...
controllers.takeElementScreenshot = async function(elementId, params = {}) {
  // TODO 通过 elementId 更新 playwright Locator
  const image = await this.locator.screenshot(Object.assign({
    fullPage: true,
  }, params));
...
controllers.keyboard = async function({ type, args }) {
  await this.page.keyboard[type].apply(this.page.keyboard, args);
  return true;
};

controllers.mouse = async function({ type, args }) {
